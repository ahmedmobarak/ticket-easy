...
export function EventCard({ event, theme, isRtl, bookingId }) {
    ...
    function CustomButton() {
        return (
            <TouchableOpacity style={{ marginTop: 40, backgroundColor: '#A0FFFF', padding: 10, borderRadius: 20 }} onPress={() => toggleModal()}>
    ...
    const QR = ({ id }) => {
        return (
            <View style={[styles.qrContainer, { backgroundColor: themes[theme].primary }]}>
                ...
                <TouchableOpacity style={{ marginTop: 40, backgroundColor: '#A0FFFF', padding: 10, borderRadius: 20 }} onPress={() => setIsVisible(false)}>
                    <Text style={{ color: '#5F5F5F', fontSize: 16 }}>Close</Text>
                </TouchableOpacity>
            </View>
        )
    ...
    return (
        <View style={[styles.container, { backgroundColor: themes[theme].primary }]}>
            ...
            {/* NOTE: conditionaly rendered part is causing the button not working */}
